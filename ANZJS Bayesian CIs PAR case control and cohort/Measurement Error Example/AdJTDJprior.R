####################################################################################
#  Adjusted Metropolis-Hastings random-walk Sampler with Sigma* = JTDJ + prior for #
...
JTDJprior <- function(data, S=1000, prior, k, sigma, IC, B=0, thin=1){
  ...
  for(t in 1:S){   # t-loop, iteration counter
    ...
    n <- sum(data)
    ## Likelihood contribution to proposal variance ##
    D = diag(c(p1/n, p2/n,p3/n, p4/n))
    Dinv = diag(c(n/p1, n/p2, n/p3, n/p4))
    
    mu = as.vector(theta[t,]) 
    t1 <-  t(J)%*%Dinv%*%J # This is dl(q|x)/d theta^2 (likelihood contribution)
    
    ## Prior contribution to the proposal variance ##
    e1 <- (-(alpha.p-1)/p^2) - ((beta.p-1)/(1-p)^2)
    e2 <- (-(alpha.q-1)/q^2) - ((beta.q-1)/(1-q)^2)
    e3 <- (-(alpha.e-1)/e^2) - ((beta.e-1)/(1-e)^2)
    e4 <- (-(alpha.Se-1)/Se^2) - ((beta.Se-1)/(1-Se)^2)
    e5 <- (-(alpha.Sp-1)/Sp^2) - ((beta.Sp-1)/(1-Sp)^2)
    
    t2 <- diag(c(e1,e2,e3,e4,e5))
    
    V_star <- t1-t2
    V = sigma*diag(5) + (V_star)  # matrices must be of the same size to sum
    ...
    if(length(which(theta[t+1,] <= 0 | theta[t+1,] >= 1)) > 0){
      ...
      ## Calculation of contributions to acceptance probability ##  
      ...
      ## Likelihood contribution to the candidate proposal variance ##
      require(mvtnorm)
      ...
      t1.c <-  t(J.c)%*%Dinv.c%*%J.c # This is dl(q|x)/d theta^2 (likelihood contribution)
      
      ## Prior contribution to the candidate proposal variance ##
      e1.c <- (-(alpha.p-1)/p.c^2) - ((beta.p-1)/(1-p.c)^2)
      e2.c <- (-(alpha.q-1)/q.c^2) - ((beta.q-1)/(1-q.c)^2)
      e3.c <- (-(alpha.e-1)/e.c^2) - ((beta.e-1)/(1-e.c)^2)
      e4.c <- (-(alpha.Se-1)/Se.c^2) - ((beta.Se-1)/(1-Se.c)^2)
      e5.c <- (-(alpha.Sp-1)/Sp.c^2) - ((beta.Sp-1)/(1-Sp.c)^2)
      
      t2.c <- diag(c(e1.c,e2.c,e3.c,e4.c,e5.c))
      
      V.c_star <- t1.c-t2.c
      V.c <- sigma*diag(5) + V.c_star
      
      log.q <- dmvnorm(theta[t,], mean=theta[t+1,], sigma = k*solve(V.c), log=T)
      log.q.c <- dmvnorm(theta[t+1,], mean=theta[t,], sigma = k*solve(V), log=T)
...
## Running the JTDJ + prior function ##
...
Results <- JTDJprior(data=data, prior=prior, sigma=40, k=0.1, S=10)
