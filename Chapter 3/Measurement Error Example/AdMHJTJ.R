####################################################################################
#  Adjusted Metropolis-Hastings random-walk Sampler with Sigma* = JTJ for          #
...
JTJ <- function(data, S=1000, prior, sigma, k, B=0, thin=1){
  ...
  for(t in 1:S){   # t-loop, iteration counter
    ...
    V = sigma*diag(5) + t(J)%*%J  # matrices must be of the same size to sum
    ...
    if(length(which(theta[t+1,] <= 0 | theta[t+1,] >= 1)) > 0){
      ...
      V.c <- sigma*diag(5) + t(J.c)%*%J.c  # matrices must be of the same size to sum
...
Results <- JTJ(data=data, prior=prior, sigma=40, k=0.1, S=100)
