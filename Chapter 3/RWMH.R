...
#                                                                                  #
# c - vector of tuning parameters for each parameter in the order (p,q,e,Se,Sp),   #
# default = (1,1,1,1,1).                                                           #
#                                                                                  #
# sd - vector of the proposal standard deviation for each parameter in order the   # 
# (p,q,e,Se,Sp)                                                                    #
#                                                                                  #
...
RWMH <- function(data, prior, B=0, S=10000, c=rep(1,5), sd, thin=1){
  ...
  for(t in 1:S){                             # t-loop, iteration counter
    ...
    for(i in 1:r){                           # i-loop, parameter indicator 1:5 = (p,q,e,Se,Sp)
      ...
      tc[i] <- rnorm(1,mean=tt[i], sd=c[i]*sd[i])  # Candidate choosen from a normal centered on previous value  
      ...
      x=c(tc[i], p1.c, p2.c, p3.c, p4.c, p1, p2, p3, p4)
      
      if(length(which(x<0 | x>1)) > 0){ 
        prev <- tt   # if any of the probabilities fall outside [0,1] reject       
      } else {
        ...
        log.pr.c <- dbeta(tc[i], shape1=prior[i,1], shape2=prior[i,2], log=T)
        log.pr <- dbeta(tt[i], shape1=prior[i,1], shape2=prior[i,2], log=T)
        
        ##########################
        # Proposal Contributions #
        ##########################
        
        log.q <- dnorm(tt[i], mean=tc[i], sd=c[i]*sd[i], log=T)
        log.q.c <- dnorm(tc[i], mean=tt[i], sd=c[i]*sd[i], log=T)
        
        ## Acceptance Probability ##
        
        alpha <- min(1,exp(log.pr.c + log.lik.c + log.q - log.pr - log.lik - log.q.c))
    ...
    if(t > B && t %% thin == 0){    # %% = modulus arg                   
      theta <- data.frame(rbind(theta, prev, deparse.level=0))
    } 
  }
  ...
  Stats <- as.data.frame(cbind(mean, sd, Lower, median, Upper, HPDLower, HPDUpper),
                         row.names=Parameter.Names)
  Results <- list("Stats" = Stats, "Theta" = theta, "AcceptRate" = acceptR)  
  return(Results)
}  

## Running the RWMH Function ##
...
Results <- RWMH(data=data, prior=prior, c=rep(2.15,5), sd=rep(0.1,5), B=4000, S=10000)
